...
const char*bootopts_mode(void){if(token("bootmode=recovery")||token("mode=recovery")||token("single"))return "Recovery";if(token("bootmode=debug")||token("mode=debug"))return "Debug";if(token("bootmode=cli")||token("mode=cli"))return "CLI";if(token("bootmode=desktop")||token("mode=desktop"))return "Desktop";return "Normal";}
...
int bootopts_desktop(void){return token("bootmode=desktop")||token("mode=desktop");}
