...
static void command_usage(const char*name){P("Usage: ");P(name);if(streq(name,"cp")||streq(name,"mv"))P(" SOURCE DEST");else if(streq(name,"cd")||streq(name,"cat")||streq(name,"xxd")||streq(name,"hexdump")||streq(name,"touch")||streq(name,"mkdir")||streq(name,"rm")||streq(name,"nano")||streq(name,"su"))P(" OPERAND");else if(streq(name,"install-apps"))P(" PROGRAM [NAME]");else if(streq(name,"install"))P(" /dev/sda");else if(streq(name,"ping"))P(" HOST");else if(streq(name,"nslookup")||streq(name,"host"))P(" NAME");else if(streq(name,"curl"))P(" [-L] [-l] URL");else if(streq(name,"ip"))P(" route");else if(streq(name,"fill"))P(" [-b|-a]");else if(streq(name,"dd"))P(" if=FILE of=FILE [bs=N] [count=N]");else if(streq(name,"head")||streq(name,"tail"))P(" [-n LINES] FILE");else if(streq(name,"wc"))P(" [-l|-w|-c] FILE");else if(streq(name,"stat")||streq(name,"basename")||streq(name,"dirname"))P(" FILE");else if(streq(name,"truncate"))P(" -s SIZE FILE");else if(streq(name,"time")||streq(name,"ntp"))P(" [--sync|--status]");else if(service_find(name)>=0)P(" [start|stop|restart|status]");else if(streq(name,"display"))P(" [status|modes|mode WIDTHxHEIGHT]");else if(streq(name,"format"))P(" [/dev/sda]");else if(streq(name,"ls")||streq(name,"find"))P(" [PATH]");else if(streq(name,"echo"))P(" [TEXT ...]");P("\nCommon options: -h, --help, --version\n");}
static int command_authorized(const char*n,const char*a){while(*a==' ')a++;if(streq(n,"install")||streq(n,"format")||streq(n,"reboot")||streq(n,"shutdown")||streq(n,"halt")||streq(n,"chown")){if(auth_is_root())return 1;}else if(streq(n,"useradd")||streq(n,"userdel")||streq(n,"usermod")||streq(n,"mkuser")||streq(n,"mount")||streq(n,"chgrp")){if(auth_is_admin())return 1;}else if(service_find(n)>=0&&*a&&!streq(a,"status")){if(auth_is_admin())return 1;}else if(streq(n,"install-apps")){if(auth_is_admin()||auth_current_role()==AUTH_POWER)return 1;}else return 1;P(n);P(": permission denied\n");last_status=77;return 0;}
static int abi_dispatch(const char*name,const char*args){if(!streq(name,"status"))last_status=0;if(!command_authorized(name,args))return last_status;size_t n=0;while(name[n]&&n<15){active_command[n]=name[n];n++;}active_command[n]=0;const char*a=args;while(*a==' ')a++;if(streq(name,"ntp")){c_ntp(args);return last_status;}if(streq(name,"install")){c_install(args);return last_status;}if((streq(a,"-h")&&!streq(name,"free"))||streq(a,"--help")){command_usage(name);return 0;}if(streq(a,"--version")){P(name);P(" (" OS64_NAME " coreutils) " OS64_KERNEL_VERSION "\n");return 0;}for(size_t i=0;i<sizeof commands/sizeof commands[0];i++)if(streq(name,commands[i].name)){commands[i].run(args);return last_status;}P("ELF: unsupported application service\n");return 126;}
