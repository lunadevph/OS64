...
static void c_tty(const char*a);
static void c_panic(const char*a);
static void c_pstore(const char*a);
...
static void c_tty(const char*a){while(*a==' ')a++;if(streq(a,"--help")){P("Usage: tty\nPrint the terminal attached to standard input.\n");return;}if(*a){P("tty: unexpected operand\n");last_status=2;return;}P("/dev/tty0\n");}
static void c_panic(const char*a){while(*a==' ')a++;if(streq(a,"--help")){P("Usage: panic --confirm REASON\nTrigger a deliberate kernel panic for diagnostics (root only).\n");return;}if(!auth_is_root()){P("panic: permission denied\n");last_status=77;return;}if(!starts(a,"--confirm ")||!a[10]){P("panic: explicit confirmation required\nUsage: panic --confirm REASON\n");last_status=2;return;}kernel_panic(a+10);}
static void c_pstore(const char*a){while(*a==' ')a++;if(!*a||streq(a,"show")){char data[512];size_t n;if(!pstore_read(data,sizeof data,&n)){P("pstore: no persistent crash record\n");return;}for(size_t i=0;i<n;i++)display_putc(data[i]);if(n&&data[n-1]!='\n')display_putc('\n');return;}if(streq(a,"clear")){if(!auth_is_root()){P("pstore: permission denied\n");last_status=77;return;}P(pstore_clear()?"pstore: crash record cleared\n":"pstore: clear failed\n");return;}if(streq(a,"--help")){P("Usage: pstore [show|clear]\nShow or clear the persistent kernel panic record.\n");return;}P("pstore: usage: pstore [show|clear]\n");last_status=2;}
...
typedef struct{const char*name,*host,*base;}package_repository_t;
#define PACKAGE_REPOSITORY_ROW(name,host,base) {name,host,base},
static const package_repository_t package_repositories[]={OS64_PACKAGE_REPOSITORIES(PACKAGE_REPOSITORY_ROW)};
#undef PACKAGE_REPOSITORY_ROW
...
static int package_fetch(const char*relative,const char**body,size_t*length){int last=-5;for(size_t r=0;r<sizeof package_repositories/sizeof package_repositories[0];r++){char path[256];size_t n=0,i=0;while(package_repositories[r].base[n]&&n+1<sizeof path){path[n]=package_repositories[r].base[n];n++;}while(relative[i]&&n+1<sizeof path)path[n++]=relative[i++];if(relative[i])continue;path[n]=0;unsigned detail=0;int result=tls_https_get(package_repositories[r].host,path,http_output,sizeof http_output-1,length,&detail);if(result!=1){last=result;continue;}http_output[*length]=0;result=package_http_body(body,length);if(result==1)return 1;last=result;}return last;}
...
static void c_pm(const char*a){
 ...
 if(streq(action,"mirrors")){P("PRIORITY  NAME       HTTPS ENDPOINT\n");for(size_t i=0;i<sizeof package_repositories/sizeof package_repositories[0];i++){display_number(i+1);P("         ");P(package_repositories[i].name);P("  https://");P(package_repositories[i].host);P(package_repositories[i].base);display_putc('\n');}return;}
...
static size_t browser_render_buffer(const char*response,size_t length,char*out,size_t cap){
    size_t body=0,n=0;for(size_t i=0;i+3<length;i++)if(response[i]=='\r'&&response[i+1]=='\n'&&response[i+2]=='\r'&&response[i+3]=='\n'){body=i+4;break;}
    int tag=0,space=0;char name[16];size_t tn=0;
    for(size_t i=body;i<length&&n+1<cap;i++){char c=response[i];if(c=='<'){tag=1;tn=0;continue;}if(tag){if(c=='>'){name[tn]=0;tag=0;if(browser_tag(name,"p")||browser_tag(name,"br")||browser_tag(name,"div")||browser_tag(name,"h1")||browser_tag(name,"h2")||browser_tag(name,"li")){if(n&&out[n-1]!='\n')out[n++]='\n';}continue;}if(tn+1<sizeof name)name[tn++]=ascii_lower(c);continue;}if(c==' '||c=='\t'||c=='\r'||c=='\n'){space=n&&out[n-1]!='\n';continue;}if(space&&n+1<cap){out[n++]=' ';space=0;}if((unsigned char)c>=32)out[n++]=c;}
    if(n&&out[n-1]!='\n'&&n+1<cap)out[n++]='\n';
    if(cap)out[n<cap?n:cap-1]=0;
    return n;
}
static int abi_browser_fetch(const char*requested,char*out,os64_size_t capacity,os64_size_t*size){
    if(!requested||!out||!capacity||!size)return -1;
    char url[256];size_t n=0;
    if(!starts(requested,"http://")&&!starts(requested,"https://")){const char*p="https://";while(*p&&n+1<sizeof url)url[n++]=*p++;}
    for(size_t i=0;requested[i]&&n+1<sizeof url;i++)url[n++]=requested[i];
    url[n]=0;int was_secure=0;
    for(unsigned redirects=0;redirects<=5;redirects++){int secure;char host[128],path[128];if(!parse_url(url,&secure,host,path))return -8;if(was_secure&&!secure)return -9;size_t length=0;unsigned detail=0;int result=secure?tls_https_get(host,path,http_output,sizeof http_output-1,&length,&detail):network_http_get(host,path,http_output,sizeof http_output-1,&length,4000000);(void)detail;if(result!=1){if(secure)return result;return result==-2?TLS_GET_DNS:TLS_GET_TCP;}http_output[length]=0;char location[256];if(redirect_location(location,sizeof location)){was_secure=secure;n=0;if(location[0]=='/'){const char*scheme=secure?"https://":"http://";while(*scheme&&n+1<sizeof url)url[n++]=*scheme++;for(size_t i=0;host[i]&&n+1<sizeof url;i++)url[n++]=host[i];}for(size_t i=0;location[i]&&n+1<sizeof url;i++)url[n++]=location[i];url[n]=0;continue;}*size=(os64_size_t)browser_render_buffer(http_output,length,out,(size_t)capacity);return 1;}return -10;
}
...
static void c_install(const char*a){
    ...
    fs_file_t kernel,boot,mini,manager;
    if(!fs_find("boot/kernel.bin",&kernel)||!fs_find("boot/os64-boot.img",&boot)||!fs_find("boot/mini64/mini64.bin",&mini)||!fs_find("boot/os64.cfg",&manager)){install_fail("Checking installation media","kernel, Mini64, or boot manager payload is missing");return;}
    ...
    if(!disk_store_large("KERNEL.BIN",kernel.data,kernel.size)||!disk_store_large("INITRD.BIN",is,(size_t)(ie-is))||!disk_store_large("MINI64.BIN",mini.data,mini.size)||!disk_store("GRUB.CFG",manager.data,manager.size)){install_fail("Installing system files","kernel, Mini64, boot policy, or initramfs write failed");return;}
...
static void execute(char*line){
    ...
    *args=0;
    ...
    if(bootopts_cli()&&(streq(requested,"desktop")||streq(requested,"sysmgr")||streq(requested,"filemm")||streq(requested,"fileman")||streq(requested,"netcfg")||streq(requested,"usercfg")||streq(requested,"setup")||streq(requested,"teteris"))){last_status=126;P(shell_name);P(": ");P(requested);P(": full-screen application disabled in CLI boot mode\n");return;}
...
int kernel_shell_execute(char *line){if(!line)return 2;execute(line);return last_status;}
const char *kernel_shell_user(void){return current_user;}
const char *kernel_shell_cwd(void){return cwd;}
...
void kernel_main(uint64_t multiboot){
    ...
    disk_init();pstore_init();boot_storage_ready=disk_formatted();
    ...
    start_boot_services();boot_summary();
    if(bootopts_desktop()){
        if(!desktop_run())P("desktop: framebuffer mode unavailable\n");
    }
    show_motd();shell_loop();
}
