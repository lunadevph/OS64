...
typedef struct{const char*name,*version,*category,*license,*upstream;int essential;const char*description;}package_t;
#define PACKAGE_ROW(name,version,category,license,upstream,essential,description) {name,version,category,license,upstream,essential,description},
...
static int package_install(const package_t*p){char source[96],target[96];if(package_installed(p->name))return 1;if(!varfs_mounted())return -1;package_path(source,"usr/share/os64/packages/",p->name);package_path(target,"/var/apps/",p->name);fs_file_t payload;if(!fs_find(source,&payload)||payload.type=='5'||payload.size>sizeof installed_image)return -2;if(!varfs_store(target,payload.data,payload.size)||!varfs_chmod(target,0755)||!varfs_sync())return -3;return 0;}
static void get_help(void){P("Usage: get COMMAND [PACKAGE]\n\nManage packages from packages/packages.json on the OS64 installation media.\n\nCommands:\n  list               list available packages\n  info PACKAGE       show package information\n  install PACKAGE    install a package into /var/apps\n  install essentials install the essential package set\n  remove PACKAGE     remove an installed package\n  status             show installed package state\n  update             validate the local package catalog\n\nOptions:\n  --help             display this help\n  --version          display version information\n");}
static void c_get(const char*a){
 ...
 if(streq(action,"update")){P("get: packages/packages.json catalog verified: ");display_number(OS64_PACKAGE_COUNT);P(" packages available\n");return;}
 ...
 if(streq(action,"install")&&streq(name,"essentials")){unsigned installed=0;if(!varfs_mounted()){P("get: /var is not mounted; install OS64 or format the data disk first\n");last_status=1;return;}for(size_t i=0;i<sizeof packages/sizeof packages[0];i++)if(packages[i].essential){int result=package_install(&packages[i]);if(result<0){P("get: failed to install ");P(packages[i].name);display_putc('\n');last_status=74;return;}if(result==0){P("get: installed ");P(packages[i].name);display_putc('\n');installed++;}}P("get: essential package set ready (");display_number(installed);P(" newly installed)\n");return;}
 const package_t*p=package_find(name);if(!name[0]||!p){P("get: package '");P(name);P("' was not found\n");last_status=1;return;}
 if(streq(action,"info")){P("Package:   ");P(p->name);P("\nVersion:   ");P(p->version);P("\nCategory:  ");P(p->category);P("\nLicense:   ");P(p->license);P("\nUpstream:  ");P(p->upstream);P("\nEssential: ");P(p->essential?"yes\n":"no\n");P("Source:    OS64 installation media\nState:     ");P(package_installed(p->name)?"installed\n":"not installed\n");P("About:     ");P(p->description);display_putc('\n');return;}
 if(streq(action,"install")){P("get: installing ");P(name);P(" ");P(p->version);P("...\n");int result=package_install(p);if(result==1){P("get: ");P(name);P(" is already installed\n");return;}if(result==-1)P("get: /var is not mounted; install OS64 or format the data disk first\n");else if(result==-2)P("get: package payload is missing or too large\n");else if(result<0)P("get: installation failed\n");else{P("get: installed /var/apps/");P(name);P("\n");return;}last_status=result==-1?1:74;return;}
