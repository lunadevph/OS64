...
static unsigned char installed_image[16384];
static char http_output[8192];
...
static void get_help(void){P("Usage: get COMMAND [PACKAGE]\n\nManage packages from the OS64 installation-media repository.\n\nCommands:\n  list             list available packages\n  info PACKAGE     show package information\n  install PACKAGE  install a package into /var/apps\n  remove PACKAGE   remove an installed package\n  status           show installed package state\n  update           verify the local package catalog\n\nOptions:\n  --help           display this help\n  --version        display version information\n");}
static void c_get(const char*a){
 while(*a==' ')a++;
 if(!*a||streq(a,"--help")){get_help();return;}
 if(streq(a,"--version")){P("OS64 get " OS64_KERNEL_VERSION "\n");return;}
 char action[16],name[16];size_t n=0;while(*a&&*a!=' '&&n+1<sizeof action)action[n++]=*a++;action[n]=0;while(*a==' ')a++;n=0;while(*a&&*a!=' '&&n+1<sizeof name)name[n++]=*a++;name[n]=0;while(*a==' ')a++;if(*a){P("get: too many arguments\n");last_status=2;return;}
 if(streq(action,"list")||streq(action,"status")){P("PACKAGE    VERSION  STATE       DESCRIPTION\n");for(size_t i=0;i<sizeof packages/sizeof packages[0];i++){P(packages[i].name);size_t z=0;while(packages[i].name[z])z++;while(z++<11)display_putc(' ');P(packages[i].version);P("      ");P(package_installed(packages[i].name)?"installed   ":"available   ");P(packages[i].description);display_putc('\n');}return;}
 if(streq(action,"update")){P("get: local catalog verified: ");display_number(sizeof packages/sizeof packages[0]);P(" packages available\n");return;}
 if(!streq(action,"info")&&!streq(action,"install")&&!streq(action,"remove")){P("get: unknown command '");P(action);P("'\n");get_help();last_status=2;return;}
 const package_t*p=package_find(name);if(!name[0]||!p){P("get: package '");P(name);P("' was not found\n");last_status=1;return;}
 if(streq(action,"info")){P("Package: ");P(p->name);P("\nVersion: ");P(p->version);P("\nSource:  OS64 installation media\nState:   ");P(package_installed(p->name)?"installed\n":"not installed\n");P("About:   ");P(p->description);display_putc('\n');return;}
 if(streq(action,"install")){if(package_installed(name)){P("get: ");P(name);P(" is already installed\n");return;}if(!varfs_mounted()){P("get: /var is not mounted; install OS64 or format the data disk first\n");last_status=1;return;}char source[96],target[96];package_path(source,"usr/share/os64/packages/",name);package_path(target,"/var/apps/",name);fs_file_t payload;if(!fs_find(source,&payload)||payload.type=='5'||payload.size>sizeof installed_image){P("get: package payload is missing or too large\n");last_status=74;return;}P("get: installing ");P(name);P(" ");P(p->version);P("...\n");if(!varfs_store(target,payload.data,payload.size)||!varfs_chmod(target,0755)||!varfs_sync()){P("get: installation failed\n");last_status=74;return;}P("get: installed /var/apps/");P(name);P("\n");return;}
 if(streq(action,"remove")){char target[96];package_path(target,"/var/apps/",name);if(!package_installed(name)){P("get: ");P(name);P(" is not installed\n");last_status=1;return;}if(!varfs_remove(target)||!varfs_sync()){P("get: removal failed\n");last_status=74;return;}P("get: removed ");P(name);P("\n");return;}
}
...
static void command_usage(const char*name){P("Usage: ");P(name);if(streq(name,"cp")||streq(name,"mv"))P(" SOURCE DEST");else if(streq(name,"cd")||streq(name,"cat")||streq(name,"xxd")||streq(name,"hexdump")||streq(name,"touch")||streq(name,"mkdir")||streq(name,"rm")||streq(name,"nano")||streq(name,"su"))P(" OPERAND");else if(streq(name,"install-apps"))P(" PROGRAM [NAME]");else if(streq(name,"get"))P(" COMMAND [PACKAGE]");else if(streq(name,"install"))P(" /dev/sda");else if(streq(name,"ping"))P(" HOST");else if(streq(name,"nslookup")||streq(name,"host"))P(" NAME");else if(streq(name,"curl"))P(" [-L] [-l] URL");else if(streq(name,"ip"))P(" route");else if(streq(name,"fill"))P(" [-b|-a]");else if(streq(name,"dd"))P(" if=FILE of=FILE [bs=N] [count=N]");else if(streq(name,"head")||streq(name,"tail"))P(" [-n LINES] FILE");else if(streq(name,"wc"))P(" [-l|-w|-c] FILE");else if(streq(name,"stat")||streq(name,"basename")||streq(name,"dirname"))P(" FILE");else if(streq(name,"truncate"))P(" -s SIZE FILE");else if(streq(name,"time")||streq(name,"ntp"))P(" [--sync|--status]");else if(service_find(name)>=0)P(" [start|stop|restart|status]");else if(streq(name,"display"))P(" [status|modes|mode WIDTHxHEIGHT]");else if(streq(name,"format"))P(" [/dev/sda]");else if(streq(name,"ls")||streq(name,"find"))P(" [PATH]");else if(streq(name,"echo"))P(" [TEXT ...]");P("\nCommon options: -h, --help, --version\n");}
static int command_authorized(const char*n,const char*a){while(*a==' ')a++;if(streq(n,"install")||streq(n,"format")||streq(n,"reboot")||streq(n,"shutdown")||streq(n,"halt")||streq(n,"chown")){if(auth_is_root())return 1;}else if(streq(n,"useradd")||streq(n,"userdel")||streq(n,"usermod")||streq(n,"mkuser")||streq(n,"mount")||streq(n,"chgrp")){if(auth_is_admin())return 1;}else if(service_find(n)>=0&&*a&&!streq(a,"status")){if(auth_is_admin())return 1;}else if(streq(n,"install-apps")||(streq(n,"get")&&(starts(a,"install ")||starts(a,"remove ")))){if(auth_is_admin()||auth_current_role()==AUTH_POWER)return 1;}else return 1;P(n);P(": permission denied\n");last_status=77;return 0;}
static int abi_dispatch(const char*name,const char*args){if(!streq(name,"status"))last_status=0;if(!command_authorized(name,args))return last_status;size_t n=0;while(name[n]&&n<15){active_command[n]=name[n];n++;}active_command[n]=0;const char*a=args;while(*a==' ')a++;if(streq(name,"ntp")){c_ntp(args);return last_status;}if(streq(name,"get")){c_get(args);return last_status;}if(streq(name,"install")){c_install(args);return last_status;}if((streq(a,"-h")&&!streq(name,"free"))||streq(a,"--help")){command_usage(name);return 0;}if(streq(a,"--version")){P(name);P(" (" OS64_NAME " coreutils) " OS64_KERNEL_VERSION "\n");return 0;}for(size_t i=0;i<sizeof commands/sizeof commands[0];i++)if(streq(name,commands[i].name)){commands[i].run(args);return last_status;}P("ELF: unsupported application service\n");return 126;}
...
static unsigned long abi_system_query(const char*n){if(!n)return 0;if(streq(n,"process.interrupted"))return process_cancel_requested();if(streq(n,"time.monotonic_ms"))return (unsigned long)(timed_monotonic_ns()/1000000ull);if(streq(n,"memory.total_kib"))return memory_total_kib();if(streq(n,"memory.free_bytes"))return memory_free_bytes();if(streq(n,"network.ready"))return network_ready();if(streq(n,"disk.mounted"))return disk_mounted();if(streq(n,"var.mounted"))return varfs_mounted();if(streq(n,"users.count"))return auth_user_count();if(streq(n,"terminal.serial"))return display_serial_available();if(streq(n,"terminal.unicode"))return 1;if(starts(n,"service.")){int id=service_find(n+8);return id>=0?(unsigned long)service_running(id):0;}return 0;}
static const os64_api_t app_api={OS64_ABI_VERSION,abi_dispatch,abi_write,abi_putc,abi_read_file,abi_write_file,abi_allocate,abi_clock_get,abi_user,abi_cwd,abi_terminal_acquire,abi_terminal_release,abi_terminal_key,abi_terminal_cell,abi_terminal_cursor,abi_terminal_width,abi_terminal_height,abi_read_directory,abi_system_query,abi_terminal_poll_key,abi_getuid,abi_getgid,abi_setuid,abi_setgid,abi_check_permission,abi_get_groups,abi_is_admin,abi_is_root};
...
void kernel_main(uint64_t multiboot){
    ...
    memory_init(multiboot);service_init();if(!bootopts_initrd_start()||!bootopts_initrd_end()||bootopts_initrd_end()<=bootopts_initrd_start())kernel_panic("INITRD_NOT_FOUND");fs_mount(bootopts_initrd_start(),bootopts_initrd_end());vfs_init();boot_filesystem_mounted=1;configure_shell();
    display_color(0x0b);
    P("\n  ____   _____  __   _  _\n");
    P(" / __ \\ / ___/ / /_ | || |\n");
    P("| |  | |\\__ \\| '_ \\| || |_\n");
