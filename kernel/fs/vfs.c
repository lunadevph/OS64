...
int vfs_write(const char*p,const unsigned char*d,size_t size,size_t*written){if(!p||(!d&&size)||!written)return VFS_WRITE_ERROR;*written=0;if(equal(p,"/dev/full"))return VFS_WRITE_NO_SPACE;if(equal(p,"/dev/null")||equal(p,"/dev/zero")||equal(p,"/dev/empty")){*written=size;return VFS_WRITE_OK;}if(equal(p,"/dev/random")||equal(p,"/dev/urandom")){if(!random_mix(d,size))return VFS_WRITE_ERROR;*written=size;return VFS_WRITE_OK;}if((starts(p,"/home/")||starts(p,"/mnt/os64/"))&&size<=512&&disk_store(p,d,(uint32_t)size)){*written=size;return VFS_WRITE_OK;}if(native_path(p)&&size<=16384&&varfs_store(p,d,size)){*written=size;return VFS_WRITE_OK;}return VFS_WRITE_ERROR;}
