...
int tls_https_get(const char*host,const char*path,char*output,size_t capacity,size_t*length,unsigned*detail){
    ...
    for(unsigned long cycles=0;cycles<200000;cycles++){
        ...
        if(state&BR_SSL_SENDREC){size_t n;unsigned char*p=br_ssl_engine_sendrec_buf(&client.eng,&n);if(tls_socket_send(&socket,p,n)!=TLS_PLATFORM_OK){if(detail)*detail=1000u+state;tls_socket_close(&socket);return TLS_GET_IO;}br_ssl_engine_sendrec_ack(&client.eng,n);continue;}
