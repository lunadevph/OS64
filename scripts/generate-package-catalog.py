...
for package in document["packages"]:
    ...
    seen.add(name)
    rows.append((name, version, category, license_id, upstream,
...
for index, row in enumerate(rows):
    name, version, category, license_id, upstream, essential, description = row
    suffix = " \\" if index + 1 < len(rows) else ""
    lines.append(f' X("{name}","{version}","{category}","{license_id}","{upstream}",{1 if essential else 0},"{description}"){suffix}')
